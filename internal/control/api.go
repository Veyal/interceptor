...
var apiRoutes = []apiRoute{
	...
	{"POST", "/api/checks/test", "Compile + run a passive check without saving. Response: {findings:[...]} or {error}"},
	...
	{"POST", "/api/findings", "Create a curated finding"},
	{"GET", "/api/findings/{id}", "Get one finding"},
	{"PATCH", "/api/findings/{id}", "Update a finding (only fields sent are changed)"},
	{"DELETE", "/api/findings/{id}", "Permanently delete a finding"},
	{"POST", "/api/findings/{id}/flows", "Attach a flow as PoC evidence to a finding (optional position)"},
	...
	{"POST", "/api/authz/cross-host-replay", "Replay a JWT (from a flow or explicit token) against every in-scope host — detects cross-host token confusion"},
	...
	{"POST", "/api/ai/assist/stream", "Streaming variant of ai/assist (SSE, token-by-token)"},
	...
	{"POST", "/api/flows/delete", "Delete flows by id ({ids:[]}); content-addressed bodies are untouched (GC separately)"},
	{"POST", "/api/flows/purge", "Purge flows by host pattern ({hosts:[],mode:delete|keepOnly}); runs GC in the background. Response: {deleted}"},
